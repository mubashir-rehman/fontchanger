...
#BADFONTS2=(find $MIRROR/system/fonts -maxdepth 1 -xtype f ! -name $BADFONTS)
#######################################################################################################
#                                               LOGGING                                               #
#######################################################################################################
# Loggers
...
while ((i++ < $choice4 - 1)); do
  cp -rf $FCDIR/Fonts/Custom/$choice2 "$FCDIR/Fonts/Custom/$choice2/${GOODFONT[$i]}"
#  i=$((i+1))
done
unset IFS
...
truncate -s 0 $CFONT
echo -n "CURRENT=$choice2" >> $CFONT
if [ -f "$FCDIR/Fonts/Custom/$choice2.zip" ] && [ -d $MODPATH/system/fonts ]; then
  font_reboot_menu
else
  echo -e "${R}[!] FONT WAS NOT APPLIED [!]${N}"
  echo -e "${R} PLEASE TRY AGAIN${N}"
...
apply_font() {
nofont=(
$(find -L $MIRROR/system/fonts -maxdepth 1 -type f -name "$GOODFONTS.ttf" -o -name "$GOODFONTS.ttc" | sed 's#.*/##' | sort -r)
)
choice4=$(ls $MIRROR/system/fonts | wc -l)
echo -e "${B}Applying Font. Please Wait...${N}"
sleep 2
...
unzip -o "$FCDIR/Fonts/$choice2.zip" 'system/*' -d $MODPATH >&2 
i=-$(ls $MODPATH/system/fonts | wc -l)
IFS=$'\n'
while ((i++ < $choice4 - 1)); do
  cp -rf $MODPATH/system/fonts "$MODPATH/system/fonts/${nofont[$i]}"
#  i=$((i+1))
done
unset IFS
...
IFS=$'\n'
while ((i++ < $choice4 - 1)); do
  cp -f $FCDIR/Fonts/Custom/$choice2/$choice3 "$FCDIR/Fonts/Custom/$choice2/${nofont[$i]}"
#  i=$((i+1))
done
unset IFS
cp -f $FCDIR/Fonts/Custom/$choice2/* $MODPATH/system/fonts > /dev/null 2>&1
